...
if (!class_exists('preg')) {
  class preg {
    ...
    public static function match ($subject, $pattern, &$matches=NULL, $flags=NULL, $offset=NULL) {
      ...
      $delimiter = utf8::substr($pattern,0 ,1);
      $modifiers = utf8::substr($pattern, utf8::strrpos($pattern, $delimiter));
      if (utf8::strpos ($modifiers, 'g') !== FALSE) {
        // remove g modifier but use preg_match_all
        $modifiers = str_replace('g', '',$modifiers);

        $pattern = utf8::substr($pattern, 0, utf8::strrpos($pattern, $delimiter)) . $modifiers;
    ...
    protected static function set_u_modifier($pattern, $add = TRUE) {
      ...
      if ($add) {
        if (utf8::strpos ($modifiers, 'u') === FALSE)
        ...
        if (utf8::strpos($modifiers, 'u') !== FALSE) {
          ...
          $pattern = utf8::substr($pattern, 0, utf8::strrpos($pattern, $delimiter)+1) . $modifiers;
        }
      }
