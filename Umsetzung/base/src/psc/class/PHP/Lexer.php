...
class PHPLexer extends Object {
  ...
  protected function scan($source) {
    ...
    foreach (token_get_all($source) as $token) {
      ...
      if (trim($t['value']) == '' && mb_strpos($t['value'],$this->eol) !== FALSE) // leer und mit whitespace
  ...
  public function loadState() {
    if (is_array($this->savedState)) {
      ...
      throw new Exception('Es wurde kein Statut mit saveState() gespeichert der jetzt geladen werden kann');
    }
  }
