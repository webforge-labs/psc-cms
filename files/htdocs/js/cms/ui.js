           ...
           /* wir holen uns alle items aus dem formular mit den nötigen informationen und bauen daraus ein paket welches wir per ajax posten */
            ...
            $form.ajaxSubmit({
              ...
              beforeSend: function(jqXHR, settings) {
                // dies ist vorwärtskompabilität und wird vielleicht niemals benutzt (weil wir oben shon aussteigen)
                $form.find('input[class="psc-cms-ui-http-header"]').each(function () {
                  // siehe auch FormTesterFrontend
                  var $input = $(this);
                  jqXHR.setRequestHeader($input.attr('name'), $input.attr('value'));
                });
              },
              success: function(data) {
                if (data.status == 'ok') {
...
$(window).load(function() {
  ...
  $( '#drop-contents .ui-accordion-header span.psc-cms-ui-icon-secondary').click(function (e) {
    /* alle öffnen*/
    e.preventDefault();
    e.stopPropagation(); // nicht akkordion zuklappen
    var $link = $(this);
      
    var delay = 40;
    $link.closest('.ui-accordion-header').next('.ui-accordion-content').find('a.psc-cms-ui-drop-content-ajax-call').each(function (i) {
      var $target = $(this), id = Psc_getGUID($target);
      setTimeout(function () {
        $.pscUI('tabs','ui.openContent',$tabs, id, $target.attr('href'), $target.text(), $target);
      }, i * delay);
    });
