...
use \Psc\CSS\CSS,
    \Psc\CSS\Manager AS CSSManager,
    \Psc\JS\Manager AS JSManager,
    \Psc\JS\JS,
    \stdClass
;


class Page extends \Psc\OptionsObject {
  ...
  public function __construct() {
    ...
    $this->attachCSSManager(CSS::getManager());
    $this->attachJSManager(JS::getManager());
  }
  ...
  public function removeMeta() {
    foreach ($this->head->content as $itemKey => $item) {
      if ($item instanceof Tag && $item->getTag() == 'meta' && ($item->getAttribute('name') == $name || $item->getAttribute('http-equiv') == $name)) {
        unset($this->html->head->content[$itemKey]);
        break;
      }
    }
  }
  ...
  public function __toString() { return (string) $this->getHTML(); }
