...
class HitchManager
{
  ...
  protected function parseObject(\SimpleXmlElement $xml, ClassMetadata $metadata)
  {
    ...
    $this->parseAnyElements($xml, $metadata, $obj);
  ...
  protected function parseEmbeds(\SimpleXmlElement $xml, ClassMetadata $metadata, $obj)
  {
    foreach($metadata->getEmbeds() as $nodeName => $info){
      ...
      if ($tempXml == NULL) {
        throw new \Psc\Exception('nodeName: \''.$nodeName.'\' ist nicht in "'.$xml->saveXML().'"');
      }
  ...
  protected function parseLists(\SimpleXmlElement $xml, ClassMetadata $metadata, $obj)
  {
    foreach($metadata->getLists() as $nodeName => $info) {
      ...
      list($property, $wrapperNode, $listMetadata) = $info;

      $tempList = array();
      if(!is_null($wrapperNode) && is_string($wrapperNode)) {
        /* wrapperMode
        ...
        */
        foreach($xml->$nodeName as $item){
          $tempList[] = $this->parseObject($item, $listMetadata);
        }
      }
   ...
   * @param stdClass $obj
   */
  protected function parseAnyElements(\SimpleXmlElement $xml, ClassMetadata $metadata, $obj)
  {
    foreach($metadata->getAnyElements() as $name => $info){
      list($property,$list) = $info;
      
      $node = $xml->$name;
      $items = $list ? $node->children() : $node;
      ...
      foreach ($items as $itemName => $item) {
        ...
        if ($list) {
          $obj->addAnyElement(new Mapping\AnyElement($itemName, $value, $attributes, $node));
        } else {
          $obj->setAnyElement(new Mapping\AnyElement($itemName, $value, $attributes, $node));
        }
      }
    }
  }
