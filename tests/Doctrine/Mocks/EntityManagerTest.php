...
class EntityManagerTest extends \Psc\Doctrine\DatabaseTest {
  ...
  protected $em; // Real EntityManager (aber auch ManagerMock)
  
  public function setUp() {
    $this->con = 'tests';
    $this->chainClass = 'Psc\Doctrine\Mocks\EntityManager';
    parent::setUp();
    $this->emm = $this->createEntityManager();
    $this->em = $this->getEntityManagerMock(); // der ist dann "schön" gebaut, mit Metadaten und so
  }
  ...
  public function testGetRepositoryMock() {
    $this->loadEntity('Psc\Doctrine\TestEntities\Tag');
    
    $repository = $this->em->getRepositoryMock($this, 'Psc\Doctrine\TestEntities\Tag');
