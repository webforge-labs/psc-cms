...
class DeployerTest extends \Psc\Code\Test\Base {
  ...
  public function setUp() {
    ...
    $this->deployments = new \Webforge\Common\System\Dir('D:\www\deployments\\');
    
    $container = new \Webforge\Framework\Container();
    $container->initLocalPackageFromDirectory(new \Webforge\Common\System\Dir(__DIR__.DIRECTORY_SEPARATOR));
    
    
    $this->deployer = clone $this->emptyDeployer
        = new Deployer(
            $this->deployments,
            $container,
            $container->getLocalProject(),
