...
class SimpleURLTest extends \Psc\Code\Test\Base {
  ...
  public function testAPI($sURL, $expectedURL, $scheme, $hostParts, $path, $query) {
    ...
    if ($url->getScheme() === 'http') {
      ...
      $this->assertEquals($url->getScheme(), SimpleURL::HTTP);
    }

    if ($url->getScheme() === 'https') {
      $this->assertTrue($url->isHTTPs(), $verbose);
      $this->assertEquals($url->getScheme(), SimpleURL::HTTPS);
    }
    
    /* host und host parts */
    $this->assertEquals($hostParts, $url->getHostParts(), $verbose);
    $this->assertEquals(implode('.',$hostParts), $url->getHost(), $verbose);
    $this->assertInstanceOf('Psc\Net\HTTP\SimpleURL',$url->getHostURL(), $verbose);
  ...
  public function provideTestApi() {
    $urls = array();
    $urls[0] = array('http://stechuhr.ps-webforge.com/index.php',
                     NULL,
                     'http',
    ...
    $urls[1] = array('http://sebastian-bergmann.de/archives/797-Global-Variables-and-PHPUnit.html',
                     NULL,
                     'http',
    ...
    $urls[2] = array('http://tiptoi.philipp.zpintern/test',
                     NULL,
                     'http',
    ...
    $urls[3] = array('http://www.google.com/',
                     NULL,
                     'http',
    ...
    $urls[4] = array('https://www.google.com/analytics',
                     NULL,
                     'https',
                     array('www','google','com'),
                     array('analytics'),
    ...
    $urls[5] = array('http://www.google.de/search?q=symfony+request+handler&ie=utf-8',
                     NULL,
                     'http',
                     array('www','google','de'),
                     array('search'),
    ...
    $urls[6] = array('http://127.0.0.1:8888/',
                     NULL,
                     'http',
                     array('127','0','0','1'),
                     array(),
    ...
    $urls[7] = array('http://ongaku.de/?lang=en',
                     NULL,
                     'http',
                     array('ongaku','de'),
                     array(),
    ...
    $urls[8] = array('http://stechuhr.ps-webforge.com/index.php/path/to/heaven?lang=nix',
                     NULL,
                     'http',
                     array('stechuhr','ps-webforge','com'),
    ...
    $urls[9] = array('http://stechuhr.ps-webforge.com/index.php/path/to/heaven/?lang=nix',
                     NULL,
                     'http',
                     array('stechuhr','ps-webforge','com'),
                     array('index.php','path','to','heaven'),
  ...
  public function testNoPartsException() {
    new SimpleURL('myproject.dev1.domain');
  }
